...
class MCTS():
    ...
    def choose_move(self):
        ...
        # board = chess.Board(self.root.fen)
        # with torch.no_grad():
        #     policy = self.net(torch.from_numpy(State(board).encode_board()[np.newaxis]).to(device))[0][0]
        #     maxIndex = torch.argmax(policy)
        #     print(policy[MOVE_LOOKUP[selected[0].uci()]], policy[maxIndex], maxIndex, list(MOVE_LOOKUP)[maxIndex])
        # del policy
        # del board
